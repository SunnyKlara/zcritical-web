...
> ⚠️ **2026-05-27 战略调整**：用户决定不快速上线，进入 **Phase 0 底座加固期（3 个月）**。
> 见 [`STRATEGY.md`](./STRATEGY.md)。
>
> 本文档的 M-Setup-2 ~ M-Setup-7 仍然有效，但**不再是阻塞 v1.0 上线的优先项**。
> Phase 0 期间这些是**和加固工作并行**的副线，等加固完成后再统一收口。
>
> 当前主线：
>
> - W1 文档与协作框架建立（**今日完成**）
> - W2 起：安全加固周（W4 主导）+ 后端基础重构（W3 主导）
> - 详见 [`STRATEGY.md`](./STRATEGY.md) §3 与 [`WORKSTREAMS.md`](./WORKSTREAMS.md)
...
**域名**：`zcritical.co`（Cloudflare Registrar 注册，自动续费 $26/年，2027-05-28 到期）

**完成动作**：

- 全代码库批量替换 `critical.bike` → `zcritical.co`（25 个文件）
- `.env.example` / `render.yaml` / OG SVG / manifest 全部同步
- 邮件模板里 `support@` `privacy@` `legal@` `dev@` `security@` `admin@` 全部更新
- 测试 41/41 通过，typecheck 全绿

**未来 API 子域名约定**：`api.zcritical.co` (后端) / `cdn.zcritical.co` (M5 起 R2)
...
| 模块                  | 状态      | 备注           |
| --------------------- | --------- | -------------- |
| M-Setup-1 域名        | ✅ 完成   | zcritical.co   |
| M-Setup-2 MongoDB     | ⏳ 进行中 | **现在做**     |
